/*
 upload-chitu-files.cpp - ESP3D http handle
   based on upload-mks-file.cpp
...
// Chitu files uploader handle
void HTTP_Server::ChituFileupload() {
  static uint32_t fragmentID = 0;
  static uint8_t buf2send[CHITU_FRAME_DATA_MAX_SIZE];
    ...
    if (upload.status == UPLOAD_FILE_START) {
      ...
      if (_webserver->hasArg("rpath")) {
        ...
        if (filename.startsWith("USB:") || filename.startsWith("SD:")) {
          ...
          ChituService::sendGcodeFrame(cmd.c_str());
          ESP3DHal::wait(10);
        }
      }
      ...
      if (ChituService::sendFirstFragment(filename.c_str(), fileSize)) {
        ChituService::uploadMode();
      }
      ...
      if (_upload_status == UPLOAD_STATUS_ONGOING) {
        ...
        while ((currentsize > 0) && (_upload_status == UPLOAD_STATUS_ONGOING)) {
          while ((buf2sendlen < CHITU_FRAME_DATA_MAX_SIZE) && currentsize > 0) {
            buf2send[buf2sendlen] = currentBuffer[0];
            buf2sendlen++;
            currentsize--;
            currentBuffer++;
          }
          if (buf2sendlen == CHITU_FRAME_DATA_MAX_SIZE) {
            esp3d_log("Send %d chars in Fragment %d", buf2sendlen, fragmentID);
            if (ChituService::sendFragment(buf2send, buf2sendlen, fragmentID)) {
              buf2sendlen = 0;
              fragmentID++;
            } else {
              _upload_status = UPLOAD_STATUS_FAILED;
      ...
      if (_upload_status == UPLOAD_STATUS_ONGOING) {
        ...
        fragmentID = ChituService::getFragmentID(fragmentID, true);
        esp3d_log("Send %d chars in Fragment %d", buf2sendlen, fragmentID);
        if (ChituService::sendFragment(buf2send, buf2sendlen, fragmentID)) {
  ...
  if (_upload_status == UPLOAD_STATUS_FAILED) {
    cancelUpload();
    // TBC need to do that
    // ChituService::sendFragment(nullptr,0,ChituService::getFragmentID(fragmentID,
    // true));
    ChituService::commandMode();
  }
}
